...
class Helpers:
    ...
    @staticmethod
    def get_nearest_strike(price: float, adjacency_gap: int = config.ADJACENCY_GAP) -> int:
    ...
    def calculate_quantity(hedge: bool = False) -> int:
        ...
        return config.LOT_SIZE if (config.HEDGE_ONE_LOT and hedge) else config.LOT_SIZE
    ...
    def is_trading_day() -> bool:
        ...
        return today in config.TRADE_DAYS

    @staticmethod
    def is_holiday(date: datetime.date) -> bool:
        """Check if date is a market holiday using TradingCalendar"""
        return not config.TRADING_CALENDAR.is_trading_day(date)
