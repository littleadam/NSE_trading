...
class ExpiryManager:
    ...
    def _get_initial_expiries(self):
        """Identify initial weekly and far month expiries with validation"""
        try:
            all_expiries = sorted({
    ...
    def _get_all_expiries(self):
        """Get all unique expiry dates with validation"""
        try:
            return sorted({
                datetime.strptime(e, '%Y-%m-%d')
    ...
    def needs_rollover(self, hedge):
        """Determine if hedge needs rollover with threshold check"""
        if not hedge:
            return False
    ...
    def replace_expiring_hedges(self):
        """Roll over expiring weekly hedges with order verification"""
        new_hedges = []
    ...
    def _get_instrument_symbol(self, name, expiry, strike, option_type):
        """Find tradingsymbol with validation"""
    ...
    def consolidate_quantities(self):
        """Merge duplicate hedge positions with config check"""
    ...
    def _replace_far_month_hedges(self, new_expiry):
        """Replace expiring far month positions with verification"""
    ...
    def get_hedge_instruments(self, spot_price: float, gap: int) -> List[Dict]:
        """Get hedge instruments with validation"""
    ...
    def get_instruments(self, strategy_type: str, ce_strike: float, pe_strike: float, far_sell_add: bool) -> List[Dict]:
        """Get tradingsymbols for strategy execution with error handling"""
    ...
    def daily_maintenance(self):
        """Execute daily maintenance tasks with error containment"""
        try:
    ...
    def _get_nearest_expiry(self, weekly: bool = True) -> datetime:
        """Get nearest weekly or monthly expiry date with fallback"""
        try:
    ...
    def get_next_weekly_expiry(self, current_expiry: datetime) -> datetime:
        """Get next weekly expiry after current expiry with validation"""
