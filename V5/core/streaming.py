...
class DataStream:
    def __init__(self, kite_client, api_key: str, access_token: str):
        ...
        self.max_retries = 3
        self.retry_count = 0
        self.token_limit = Config.WS_TOKEN_LIMIT
    ...
    def _on_reconnect(self, ws):
        ...
        Timer(1, self.start_stream).start()
    ...
    def _on_ticks(self, ws, ticks: List[Dict]):
        ...
        with self.lock:
            ...
            try:
                # Update price data with thread-safe write
                for tick in ticks:
    ...
    def subscribe(self, tokens: List[int]):
        """Safe subscription with connection checks"""
        if not self.active:
    ...
    def get_spot_price(self) -> Optional[float]:
        """Get current Nifty spot price with thread safety"""
        with self.lock:
            try:
    ...
    def get_option_data(self, tradingsymbol: str, instrument_type: str) -> Optional[Dict]:
        """Thread-safe option data retrieval"""
