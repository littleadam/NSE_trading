...
class DataStream:
    ...
    def _is_data_stale(self, ticks: List[Dict]) -> bool:
        ...
        if not ticks:
            return False
        newest_tick = max(t['timestamp'] for t in ticks)
        return (time.time() - newest_tick.timestamp()) > 60
    ...
    def subscribe(self, tokens: List[int]):
        ...
        try:
            # Respect Zerodha's token limit with priority preservation
    ...
    def _prune_tokens(self):
        ...
        token_priority = {
            t: (2 if t == self.nifty_token else              # Highest priority for NIFTY spot
                1 if self._is_strategy_instrument(t) else    # Medium priority for strategy instruments
                0)                                           # Lowest priority for others
