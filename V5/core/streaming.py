...
class DataStream:
    def __init__(self, kite_client, api_key: str, access_token: str):
        ...
        try:
            ...
            logger.critical(f"Initial instrument load failed: {str(e)}")
            raise
    ...
    def _load_instruments(self):
        ...
        try:
            ...
            self.nifty_token = next(
                i['instrument_token'] for i in nse_instruments 
                if i['tradingsymbol'] == 'NIFTY 50'
            )
            ...
            logger.error("NIFTY 50 instrument not found in NSE listings")
            raise
        except Exception as e:
            logger.error(f"Instrument loading failed: {str(e)}")
            raise

    def _get_instrument_token(self, tradingsymbol: str, instrument_type: str) -> int:
    ...
    def _on_connect(self, ws, response):
        ...
        # Always subscribe to Nifty spot
        self.subscribe([self.nifty_token])

    def _batch_subscribe(self, tokens: list):
        ...
        with self.lock:
            for tick in ticks:
                try:
                    ...
                    logger.warning(f"Malformed tick data: {str(e)}")
                except Exception as e:
    ...
    def get_option_data(self, tradingsymbol: str, instrument_type: str) -> Dict:
    ...
    def add_strategy_instruments(self, strategies: list):
